...
class TagFolderWorkbenchSpecTest(unittest.TestCase):
    ...
    def test_playlist_application_boundary_is_preview_first_and_stale_safe(self) -> None:
        ...
        review_projection = (ROOT / "metadata-tags/src/main/java/dev/properpcloud/metadata/tags/MetadataReviewProjection.kt").read_text()
        ...
        desktop_controller = (ROOT / "desktop-app/src/main/kotlin/dev/properpcloud/desktop/DesktopController.kt").read_text()
        desktop_ui = (ROOT / "desktop-app/src/main/kotlin/dev/properpcloud/desktop/DesktopUi.kt").read_text()
        playlist_cli = (ROOT / "desktop-app/src/main/kotlin/dev/properpcloud/desktop/PlaylistCli.kt").read_text()
        ...
        self.assertIn("FolderPlaylistOrder.TITLE_NUMBER", workspace)
        self.assertIn("FolderPlaylistOrder.TITLE_NUMBER", writer)
        ...
        self.assertIn("FolderTagReviewTransition.REMOVAL_TO_EMPTY", review_projection)
        self.assertIn("FolderTagReviewTransition.ADDED_FROM_EMPTY", review_projection)
        self.assertIn("val projection: FolderTagReviewProjection", session)
        self.assertIn("val projection: FolderPlaylistReviewProjection", session)
        self.assertIn("tagReview: FolderTagReviewProjection?", desktop_controller)
        self.assertIn("playlistReview: FolderPlaylistReviewProjection?", desktop_controller)
        self.assertIn('"Frozen Earlier / Later review"', desktop_ui)
        self.assertIn('"Earlier"', desktop_ui)
        self.assertIn('"Later"', desktop_ui)
        self.assertIn('"Empty — will remove value"', desktop_ui)
        self.assertIn('"Exact playlist checkpoint', desktop_ui)
        self.assertIn('"title-number" -> FolderPlaylistOrder.TITLE_NUMBER', playlist_cli)
        self.assertIn("playlist.finalLines.forEach(output)", playlist_cli)
        self.assertIn("preview is the default and prints every exact target", boundaries["native_desktop_cli"])
        self.assertIn("leading decimal integer", playlist["sort_modes"]["title_number"])
        self.assertIn("every final M3U8 line", playlist["batch_generation"]["review_checkpoint"])
