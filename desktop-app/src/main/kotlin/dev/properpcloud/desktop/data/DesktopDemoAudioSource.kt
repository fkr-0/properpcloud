...
class DesktopDemoAudioSource(private val mediaDirectory: Path) : AudioSource {
    ...
    private val playlist = LibraryFile(id, NodeId("demo:file:summer-playlist"), music.id, "Summer demo set.m3u8", contentType = "audio/x-mpegurl", sizeBytes = 96)
    private val notes = LibraryFile(id, NodeId("demo:file:notes"), music.id, "Summer demo notes.txt", contentType = "text/plain", sizeBytes = 64)
    private val frequencies = mutableMapOf<NodeId, Int>()
    ...
    override suspend fun resolveStream(trackId: NodeId): StreamHandle {
        ...
        val file = mediaFile(track)
        if (!Files.exists(file) || Files.size(file) < 44) writeTone(file, frequencies.getValue(track.id), track.durationMillis ?: 5_000)
        return StreamHandle(file.toUri().toString(), contentType = "audio/wav")
    }

    override suspend fun inspect(nodeId: NodeId): NodeInspection {
        val node = load(nodeId)
        return NodeInspection(linkedMapOf(
    ...
    private fun track(folder: AudioFolder, name: String, disc: Int?, number: Int?, duration: Long, frequency: Int): AudioTrack =
        AudioTrack(
            sourceId = id,
            id = NodeId("demo:track:$frequency:$duration"),
            parentId = folder.id,
            name = name,
        ...
        ).also { frequencies[it.id] = frequency }

    private fun mediaFile(track: AudioTrack): Path = mediaDirectory.resolve(track.id.value.replace(':', '_') + ".wav")

    private fun writeTone(path: Path, frequency: Int, durationMillis: Long) {
        ...
        DataOutputStream(BufferedOutputStream(Files.newOutputStream(path))).use { output ->
            ...
            repeat(sampleCount) { index ->
                ...
                output.writeLittleEndianShort((sin(2 * PI * frequency * index / sampleRate) * 6_000 * envelope).toInt())
            }
        }
    }
}
...
private fun DataOutputStream.writeLittleEndianShort(value: Int) { writeByte(value and 0xff); writeByte(value ushr 8 and 0xff) }
