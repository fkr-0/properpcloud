...
- an unexpected process exit triggers one application-orchestrated restart from stable media identity and last confirmed position; paused sessions stay paused, and a manual restart remains available after failure;
- stale stream resolutions are prepared paused under a serialized load boundary and cannot become audible after a newer tab, queue, stop, or close intent wins;
- resuming/recovering reloads mpv's automatic audio output so ordinary headphone/Bluetooth hot-plug changes are re-evaluated without selecting a fixed device;
- mpv stdin and per-command IPC sockets are closed deterministically, and replaced process generations are reaped before the next one is retained;
